...
- 주문(order_master) - 1 : N - 주문상품(order_items)
- 배송(shipment_master) - 1 : N - 배송상품(shipment_items)
- 배송상품(shipment_items) - 1 : 1 - 주문상품(order_items)
...
또한, 각 주문상품에 대응하는 배송상품(shipment_items)이 만들어진다.
...
기존 배송에 포함된 배송상품(shipment_items)을 분리하여 새로운 배송을 생성하거나, 배송 미할당 상태로 되돌릴 수 있다.
...
주문 테이블(order_master, order_items)은 "주문 접수 시점의 정보"만 관리한다.

배송 상태, 송장번호, 택배사 코드 등은 배송 도메인(shipment_master, shipment_items)이 관리한다.

출고 가능 여부는 order_items.matching_status + 재고 테이블을 런타임 JOIN하여 판단한다.
...
원본 추적은 단일 원천으로 관리한다:
  ...
  - order_items.origin_order_item_id → 원주문상품 ID

CS에서 원본을 조회할 때: cs_claims.order_product_id → order_items.origin_order_item_id


### 1-7. 마스터/아이템 독립 관리

상태 컬럼(is_delivery_on_hold, is_canceled, is_deleted)은 양쪽 모두에 존재한다.
  - order_master의 상태 = 주문 전체에 적용
  - order_items의 상태 = 해당 상품에만 적용
...
order_items.matching_status와 inventory 테이블을 런타임 JOIN하여 판단한다.
...
재고(inventory)는 상품(product_items = SKU) 단위로 관리한다.
...
상품 도메인과 재고 도메인의 책임:
  ...
  - product_items    : SKU가 뭔지 (옵션 조합, 바코드, 단가)
...
| **shipment_ 접두어**    | 담당자/출고 관리 관점 (shipment_master, shipment_items, is_shipment_on_hold) |
| **코드값**              | 상태/유형은 int + common_code 참조 |
| **감사 컬럼 순서**         | created_user_id → created_at → updated_user_id → updated_at |
| **주소 타입**            | varchar(500), 인덱싱 가능 |
| **테이블 네이밍**          | 도메인 일관성: master / items 패턴 |
| **출고 가능 여부**         |	DB 저장 안 함, 런타임 재고 JOIN으로 판단 |
| **재고와 SKU 분리**       |	product_items = SKU 정의, inventory = 수량 관리 |
